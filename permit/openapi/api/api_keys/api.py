...
class AsyncAPIKeysApi(Api):
    def __init__(self, *args, **kwargs):
        ...
        self.get_environment_api_key = functools.partial(get_environment_api_key.asyncio,
                                                         client=self.client,
...
class APIKeysApi(Api):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.get_environment_api_key = functools.partial(get_environment_api_key.sync,
                                                         client=self.client,
                                                         permit_session=self.permit_session)
