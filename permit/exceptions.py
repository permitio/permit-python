...
def raise_for_error(res: HTTPValidationError | None | Any,
...
def raise_for_error_by_action(res: HTTPValidationError | None | Any,
                              object_name: str,
                              payload: str,
                              action: str = "get"):
    if action == "delete":
        raise_for_error(res, message=f"could not {action} {object_name} with key '{payload}'",
                        allow_none=True)
    elif action in ("create", "update", "remove"):
        raise_for_error(res, message=f"could not {action} {object_name} with: {payload}",
                        allow_none=False)
    else:
        raise_for_error(res, message=f"could not {action} {object_name} with key '{payload}'",
                        allow_none=False)
