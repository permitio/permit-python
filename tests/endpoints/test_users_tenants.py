...
ADMIN = RoleCreate(key=f"admin-{uuid.uuid4()}", name="Admin")
VIEWER = RoleCreate(key=f"viewer-{uuid.uuid4()}", name="Viewer")
...
async def test_users_tenants(permit: Permit):
    ...
    tenants = await permit.api.tenants.list()
    assert len(tenants) == len_original_tenants + len(CREATED_TENANTS)
    ...
    role_assignments = await permit.api.role_assignments.list()
    assert len(role_assignments) == len_original_role_assignments + 3
    ...
    # list role assignments
    role_assignments = await permit.api.role_assignments.list()
    assert len(role_assignments) == len_original_role_assignments + 2
    ...
    # list role assignments
    role_assignments = await permit.api.role_assignments.list()
    assert len(role_assignments) == len_original_role_assignments
