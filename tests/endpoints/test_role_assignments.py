...
async def create_role_assignments(permit: Permit, role_key: str, user_count: int = 10):
    ...
    with suppress_409():
        await permit.api.users.bulk_create([UserCreate(key=f"user-{index}") for index in range(user_count)])
    with suppress_409():
        await permit.api.role_assignments.bulk_assign(
            [RoleAssignmentCreate(role=role_key, user=f"user-{index}", tenant="default") for index in range(user_count)]
        )
...
async def test_list_filter_by_role_multiple(permit: Permit):
    ...
    role_assignments = await permit.api.role_assignments.list(role_key=["role-1", "role-2"])
