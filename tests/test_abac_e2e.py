...
async def test_abac_e2e(permit: Permit):
    ...
    try:
        ...
        # NOTE: everything below depends on the condition sets being compiled
        # into policy and reaching the PDP. Against a PDP that serves a static
        # policy snapshot (the local dev PDP) these decisions are not meaningful;
        # they are validated by CI against the real cloud PDP.
        logger.info("waiting for the condition sets to be compiled into policy and reach the PDP")
        ...
        await wait_until(
            ...
            f"the condition set rule granting '{sign_permission}' to users over 30 to reach the PDP",
            timeout=ABAC_PROPAGATION_TIMEOUT,
        )
