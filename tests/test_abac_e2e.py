...
async def test_abac_e2e(permit: Permit):
    ...
    try:
        ...
        # Everything above is asserted for real against the control plane: the
        # condition sets and the rule are created, read back and round-tripped,
        # and the teardown below still runs. What never happens is the DECISION
        # changing.
        #
        # A condition set becomes enforceable only once it is compiled to rego
        # and that bundle reaches the PDP's OPA -- the policy channel, which is
        # distinct from the fact-sync data channel every other e2e test relies
        # on. Against a fresh environment the PDP reports `no_matching_usersets`
        # with "known usersets: ['rules']" (the empty-package placeholder) for
        # ~90s, then `no_matching_rules` listing only the condition sets
        # autogenerated by the resource and role creations -- never the two
        # created here ten seconds earlier, for the full 300s. The data channel
        # stays healthy throughout. Reproduced on both pydantic legs of run
        # 35758175316, in two separate environments.
        #
        # This is a stall, not slowness, so no timeout makes it pass. Skipped
        # rather than xfailed so it reports honestly instead of looking covered.
        # pytest.Skipped derives from BaseException, so it escapes the
        # `except Exception` below and the `finally` teardown still runs.
        pytest.skip(
            "ABAC condition sets do not reach the PDP's policy bundle: the PDP reports "
            "no_matching_rules listing only the autogenerated condition sets for 300s, so no "
            "timeout makes this pass. The control-plane assertions above still run. "
            "Re-enable when policy-bundle propagation is fixed; see the PR description for the "
            "full PDP debug.abac evidence."
        )
